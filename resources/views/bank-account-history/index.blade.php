...
@section('title', $bankAccount->name.' Hesap Ekstresi')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('bank_accounts.index', ['bankId' => $bank->id]) }}"> {{$bank->name}}
            Hesaplar</a></li>
    <li class="breadcrumb-item active">{{ $bankAccount->name }} Hesap Ekstresi</li>
@endsection

@section('content')
    <a href="{{ route('bank_account_history.create', ['bankId' => $bank->id, 'bankAccountId' => $bankAccount->id]) }}"
       class="button-floating"><i
                    ...
                    <div class="row" style="margin-bottom: 10px;">
                        <div class="col-md-12">
                            <a style="float:right;" class="btn btn-danger"
                               href="{{ route('bank_account_history.trashed', ['bankId' => $bank->id, 'bankAccountId' => $bankAccount->id]) }}"
    ...
    <script>
        $(document).ready(function () {
            $('#main-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('bank_account_history.datatables', ['bankId' => $bank->id, 'bankAccountId' => $bankAccount->id]) }}',
                columns: [
                    {data: 'DT_RowIndex'},
                    {data: 'title'},
                    {data: 'description'},
